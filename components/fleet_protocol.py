...
def _validate_trace_report_header(value: TraceReportPayload, sample_count: int) -> None:
    ...
    if not 0 <= sample_count <= TRACE_MAX_SAMPLES:
        raise ProtocolError("payload", "TRACE_REPORT sample count must be between 0 and 15")
    if sample_count == 0:
        if value.first_sample_seq != 0:
            raise ProtocolError("payload", "empty TRACE_REPORT must have first_sample_seq zero")
        return
    if value.first_sample_seq == 0:
        raise ProtocolError("payload", "non-empty TRACE_REPORT must have a nonzero first sample")
    ...
    if value.latest_available_seq < last_sample_seq:
        raise ProtocolError("payload", "TRACE_REPORT samples exceed latest_available_seq")
...
def encode_trace_report(value: TraceReportPayload) -> bytes:
    ...
    encoded = bytearray(TRACE_REPORT_HEADER.pack(
    ...
    if samples:
        ...
        encoded.extend(TRACE_SAMPLE_ABSOLUTE.pack(
            first.uptime_ms,
            first.x_cm,
            first.y_cm,
            first.z_cm,
        ...
        for sample in samples[1:]:
            ...
            dt_ms = _range("dt_ms", sample.uptime_ms - previous.uptime_ms, 1, 0xFFFF)
            ...
            encoded.extend(TRACE_SAMPLE_DELTA.pack(
                dt_ms, dx_cm, dy_cm, dz_cm,
                sample.heading_cdeg, sample.quality, sample.flags,
            ))
            previous = sample
...
def decode_trace_report(data: bytes) -> TraceReportPayload:
    ...
    values = TRACE_REPORT_HEADER.unpack_from(payload_data)
    request_session, request_seq, trace_session, oldest_available_seq, first_sample_seq, latest_available_seq, sample_count, report_flags = values
    expected_size = TRACE_REPORT_HEADER.size
    if sample_count:
        expected_size += TRACE_SAMPLE_ABSOLUTE.size + (sample_count - 1) * TRACE_SAMPLE_DELTA.size
    if len(payload_data) != expected_size:
        raise ProtocolError("payload", "TRACE_REPORT sample count/length mismatch")
    provisional = TraceReportPayload(
        request_session, request_seq, trace_session, oldest_available_seq,
        first_sample_seq, latest_available_seq, report_flags, (),
    )
    _validate_trace_report_header(provisional, sample_count)
    if not sample_count:
        return provisional
    first = TraceSample(*TRACE_SAMPLE_ABSOLUTE.unpack_from(payload_data, TRACE_REPORT_HEADER.size))
    validate_trace_sample(first)
    samples = [first]
    previous = first
    offset = TRACE_REPORT_HEADER.size + TRACE_SAMPLE_ABSOLUTE.size
    for _ in range(1, sample_count):
        dt_ms, dx_cm, dy_cm, dz_cm, heading_cdeg, quality, sample_flags = TRACE_SAMPLE_DELTA.unpack_from(payload_data, offset)
        ...
        sample = TraceSample(
            ...
            heading_cdeg, quality, sample_flags,
        )
        validate_trace_sample(sample)
    ...
    return TraceReportPayload(
        request_session, request_seq, trace_session, oldest_available_seq,
        first_sample_seq, latest_available_seq, report_flags, tuple(samples),
    )


def encode_command(value: CommandPayload) -> bytes:
    data = bytes((_u8("command_id", value.command_id), _u8("command_flags", value.command_flags))) + bytes(value.command_body)
...
def encode_ack(value: AckPayload) -> bytes:
    ...
    if len(detail) > 0xFF:
        raise ProtocolError("payload", "ACK detail exceeds 255 bytes")
    data = ACK_HEADER.pack(
        _u32("request_session", value.request_session), _u16("request_seq", value.request_seq),
        _u8("command_id", value.command_id), _u8("status", value.status),
        _u8("reason", value.reason), len(detail),
    ) + detail
    if len(data) > MAX_PAYLOAD_LEN:
...
def decode_coordinate_frame(data: bytes) -> CoordinateFrameCommand:
    if len(data) != COORDINATE_FRAME.size:
        raise ProtocolError("payload", "SET_COORDINATE_FRAME body has invalid length")
    value = CoordinateFrameCommand(*COORDINATE_FRAME.unpack(data))
    _heading(value.startup_x_heading_cdeg)
    return value


def _goal(data_struct: struct.Struct, values: Tuple[int, ...], heading: Optional[int]) -> bytes:
    body = data_struct.pack(0, *values)
    if heading is not None:
        body = bytes((int(GoalFlags.HAS_FINAL_HEADING),)) + body[1:] + HEADING.pack(_heading(heading))
    return body


def _decode_goal(data: bytes, data_struct: struct.Struct) -> Tuple[Tuple[int, ...], Optional[int]]:
    if len(data) not in (data_struct.size, data_struct.size + HEADING.size):
        raise ProtocolError("payload", "goal body has invalid length")
    unpacked = data_struct.unpack_from(data)
    ...
    if has_heading != (len(data) == data_struct.size + HEADING.size):
        raise ProtocolError("payload", "goal heading flag/length mismatch")
    heading = HEADING.unpack_from(data, data_struct.size)[0] if has_heading else None
    if heading is not None:
        _heading(heading)
...
def _decode_points(data: bytes) -> Tuple[int, int, int, Tuple[Tuple[int, int], ...]]:
    ...
    points = tuple(POINT.unpack_from(data, POINT_REPORT_HEADER.size + i * POINT.size) for i in range(count))
