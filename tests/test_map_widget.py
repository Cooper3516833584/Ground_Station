...
class FleetMapWidgetRenderingTests(unittest.TestCase):
    ...
    def test_task_timer_starts_on_motion_and_freezes_when_car_arrives(self):
        now = [100.0]
        window = DTaskMainWindow(clock=lambda: now[0])
        drone = SimpleNamespace(
            online=True,
            stale=False,
            session=10,
        ...
        window.update_snapshot(snapshot)
        self.assertEqual("任务用时：--:--.-", window._task_timer.text())

        drone.operation_state = 3
        window.update_snapshot(snapshot)
        now[0] = 165.4
        car.operation_state = 4
        window.update_snapshot(snapshot)
        self.assertEqual("任务用时：01:05.4", window._task_timer.text())

        now[0] = 170.9
        car.operation_state = 7
        window.update_snapshot(snapshot)
        self.assertEqual("任务用时：01:10.9", window._task_timer.text())
    ...
    def test_task_timer_restarts_only_when_next_task_moves(self):
        now = [0.0]
        window = DTaskMainWindow(clock=lambda: now[0])
        drone = SimpleNamespace(
            online=True,
        ...
        now[0] = 20.0
        window.update_snapshot(snapshot)
        self.assertEqual("任务用时：00:10.0", window._task_timer.text())

        drone.operation_state = 30
        window.update_snapshot(snapshot)
        now[0] = 30.0
        drone.operation_state = 3
        window.update_snapshot(snapshot)
        ...
        car.operation_state = 4
        window.update_snapshot(snapshot)
        self.assertEqual("任务用时：00:04.2", window._task_timer.text())
        window.close()
