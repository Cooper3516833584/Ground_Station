...
`START` 后，地面站白灯满亮度闪烁 3 秒并转为 3/255 白灯常亮；从点击开始满 10 秒后才向
无人机发送任务 START。检测到无人机已解锁且高度达到配置门限后，才发送
...
场地坐标以左下角为 `(0,0)`，`+X` 向右、`+Y` 向上；无人机起飞区中心为 `(75,75)`，
小车启动后轴中心为 `(160,50)`。比赛角度约定“正上方为 0°、顺时针为正”，无人机和小车
初始均为 0°；地面站仅在 `SET_COORDINATE_FRAME` 边界把它换算为现有导航内部的
`+X=0°/逆时针为正`（即内部 90°），不修改车端导航角度语义。水源、山火和返航目标均不
